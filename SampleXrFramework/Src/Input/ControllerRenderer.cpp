...
bool ControllerRenderer::LoadModelFromGlbBuffer(const uint8_t* data, size_t size) {
  if (Model) {
    delete Model;
    Model = nullptr;
  }
  ...
  OVRFW::ModelGlPrograms programs(&ProgControllerTexture);
  OVRFW::MaterialParms materials;
  Model = OVRFW::LoadModelFile_glB("runtime_controller", reinterpret_cast<const char*>(data),
                   static_cast<int>(size), programs, materials);
  if (Model == nullptr) {
    return false;
  }
  ...
  ControllerSurface.surface = &(ControllerSurfaceDef);
  ControllerSurfaceDef.graphicsCommand.Program = ProgControllerTexture;
  return true;
}
