...
namespace CTX
{
    ...
    public:
        ...
        {
            scale_ = OVR::Vector3f(s);
            dirty_ = true;
        }
        void setScale(float x, float y, float z)
        {
            scale_ = OVR::Vector3f(x, y, z);
            dirty_ = true;
        }
        void setHpr(float hDeg, float pDeg, float rDeg);
        bool isLoaded() const { return renderer_ && renderer_->IsInitialized(); }
        void updatePose();
        void emitSurfaces(std::vector<OVRFW::ovrDrawSurface> &surfaces);

    private:
        std::unique_ptr<OVRFW::SimpleGlbRenderer> renderer_;
